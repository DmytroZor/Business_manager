...
from core.models import Address, Customer, Courier, Delivery, DeliveryStatus, Order, OrderStatus, Product, ProductBatch, User, UserRole
...
from manage.services.delivery_service import get_available_orders_for_courier, pick_up_delivery, self_assign_delivery
...
async def test_self_assign_delivery_creates_assigned_delivery(db_session):
    ...
    product = Product(
        ...
        unit="kg",
        available_quantity=Decimal("3.000"),
        is_active=True,
    )
    ...
    assert delivery.order.items[0].product_name == "Carp"
    assert delivery.courier.user.full_name == "Courier"
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_pick_up_delivery_consumes_reserved_stock_and_batches(db_session):
    customer_user = User(
    ...
    address = Address(customer_id=customer.id, street="Pickup Street", building="9", apartment=None, notes=None)
    product = Product(
        name="Sea Bass",
        sku="RIBA-PICKUP-01",
        description="Fresh sea bass",
        base_unit_price=Decimal("210.00"),
        unit="kg",
        available_quantity=Decimal("3.000"),
        is_active=True,
    )
    ...
    await pick_up_delivery(db_session, delivery.id, courier.id)

    await db_session.refresh(product)
    assert product.available_quantity == Decimal("2.000")
    assert product.reserved_quantity == Decimal("0.000")
    assert product.stock_on_hand == Decimal("2.000")

    batch = await db_session.scalar(select(ProductBatch).where(ProductBatch.product_id == product.id))
    assert batch is not None
    assert batch.available_quantity == Decimal("2.000")
